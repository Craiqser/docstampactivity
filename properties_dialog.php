...
?>
<tr>
	<td align="right" width="40%"><span class="adm-required-field"><?= GetMessage("DOCSTAMP_SOURCE_FILE") ?>:</span></td>
	<td width="60%">
		<?= \CBPDocument::ShowParameterField('file', 'source_file', $arCurrentValues['source_file'], array('size' => 30)) ?>
	</td>
</tr>
<tr>
	<td align="right" width="40%"><?= GetMessage("DOCSTAMP_CREATED_BY") ?>:</td>
	<td width="60%">
		<?= \CBPDocument::ShowParameterField('user', 'created_by', $arCurrentValues['created_by'], array('rows' => 1, 'size' => 29)) ?>
	</td>
</tr>
<tr>
	<td align="right" width="40%"><span class="adm-required-field"><?= GetMessage("DOCSTAMP_PD_ENTITY") ?>:</span></td>
	<td width="60%">
		<select name="entity_type" onchange="DOCSTAMP_changeEntityType(this.value);">
			<option value="user" <?if ($arCurrentValues['entity_type'] == 'user') echo 'selected'?>><?=GetMessage('DOCSTAMP_PD_ENTITY_TYPE_USER')?></option>
			<option value="common" <?if ($arCurrentValues['entity_type'] == 'common') echo 'selected'?>><?=GetMessage('DOCSTAMP_PD_ENTITY_TYPE_COMMON')?></option>
			<option value="folder" <?if ($arCurrentValues['entity_type'] == 'folder') echo 'selected'?>><?=GetMessage('DOCSTAMP_PD_ENTITY_TYPE_FOLDER_1')?></option>
		</select>
	</td>
</tr>
<tr id="id_entity_id_user" <?if ($arCurrentValues['entity_type'] != 'user') echo 'style="display: none"'?>>
	<td align="right" width="40%"><span class="adm-required-field"><?= GetMessage("DOCSTAMP_PD_ENTITY_ID_USER") ?>:</span></td>
	<td width="60%">
		<?
...
<tr id="id_entity_id_common" <?if ($arCurrentValues['entity_type'] != 'common') echo 'style="display: none"'?>>
	<td align="right" width="40%"><span class="adm-required-field"><?= GetMessage("DOCSTAMP_PD_ENTITY_ID_COMMON") ?>:</span></td>
	<td width="60%">
		<?
		$value = $arCurrentValues['entity_type'] == 'common'? $arCurrentValues['entity_id'] : '';
		?>
		<select name="entity_id_common">
			<option value=""><?= GetMessage("DOCSTAMP_PD_LABEL_CHOOSE") ?></option>
			<?
...
<tr id="id_entity_id_folder" <?if ($arCurrentValues['entity_type'] != 'folder') echo 'style="display: none"'?>>
	<td align="right" width="40%"><span class="adm-required-field"><?= GetMessage("DOCSTAMP_PD_ENTITY_ID_FOLDER") ?>:</span></td>
	<td width="60%">
		<?
		$value = $arCurrentValues['entity_type'] == 'folder'? $arCurrentValues['entity_id'] : '';
		$folderId = 0;
		$folderName = GetMessage('DOCSTAMP_PD_LABEL_DISK_EMPTY');
		...
		<div style="padding: 3px;">
			...
			<a href="#" id="id_entity_id_folder_clear" onclick="return DOCSTAMP_removeDiskFolder()" style="<?=$folderId?'':'display: none;'?>color: red; text-decoration: none; border-bottom: 1px dotted">x</a>
			<br/>
			<a href="#" onclick="return DOCSTAMP_showDiskDialog()" style="color: black; text-decoration: none; border-bottom: 1px dotted"><?=GetMessage('DOCSTAMP_PD_LABEL_DISK_CHOOSE')?></a>
		</div>
		<?
...
<script>
	var DOCSTAMP_changeEntityType = function(type)
	{
		var i, s, types = ['user', 'common', 'folder'];
	...
	var DOCSTAMP_removeDiskFolder = function()
	{
		BX('id_entity_id_folder_value').value = 0;
		BX('id_entity_id_folder_name').innerHTML = '<?=GetMessageJs('DOCSTAMP_PD_LABEL_DISK_EMPTY')?>';
